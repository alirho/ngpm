# پیاده‌سازی محصول

برای پیاده‌سازی یک محصول با استفاده از ابزارهای هوش می‌توان از گام‌های زیر استفاده کرد.

## طراحی و کدهای پایه

برای طراحی یک محصول می‌توان از ابزار [stitch](https://stitch.withgoogle.com) گوگل استفاده کرد. این ابزار با گرفتن پرامپت طرح مورد نظر را پیاده‌سازی و کدهای HTML و CSS (tailwind) آن را در اختیار شما قرار می‌دهد.

از این ابزار به دو روش می‌توان استفاده کرد:

1. روش یک: هیچ طرحی برای پروژه وجود ندارد
2. روش دو: طرح‌های پروژه وجود دارند
### بدون طرح

در روش نخست که هیچ طرحی برای پروژه وجود ندارد، باید با استفاده از پرامپت‌های شفاف و کامل نیازمندی‌ها خود را مشخص کنید تا stitch بتواند با جزئیات خوبی درخواست شما را پیاده‌سازی کند. اما باید این نکته را مدنظر داشته باشید که stitch در حال حاضر نمی‌تواند طرح‌های مناسب باز زبان فارسی ایجاد کند و طرح‌های ایجاد شده توسط این ابزار مناسب زبان‌ها چپ به راست است.

پس از طراحی کدهای مربوط به هر طرح در بخش code در دسترس هستند و می‌توانید از آن‌ها رونوشت بگیرید.

### با استفاده از طرح

اگر پروژه دارای طرح می‌باشد می‌توانید با بارگذاری هر تصویر و نوشتن پرامپت زیر، کدهای هر طرح را دریافت کنید.

> پرامپت: طرحی که در عکس هست را به صورت کامل و دقیق و بدون تغییر پیاده‌سازی کن.

در نظر داشته باشید دقت خروجی این گام بیش از ۸۰ درصد است و نیاز به تغییراتی جزئی می‌باشد.

روش دیگر برای دریافت کدهای پایه این است که تصویر را در [gemini](https://gemini.google.com) بارگذاری کرده و پرامپت زیر را همراه با تصویر به مدل داد.

> پرامپت: ازت می‌خوام عکسی که بارگذاری کردم با استفاده از HTML و CSS پیاده‌سازی کنی.

مزیت استفاده از gemini این است که برای پیاده‌سازی CSS از کتابخانه tailwind استفاده نمی‌کند.

## بازبینی و بازنویسی کدهای پایه

در گام دوم پیاده‌سازی نیاز است تا کدهایی که در گام نخست از ابزار stitch دریافت شده است مورد بازبینی و بازنویسی (refactor) قرار گیرند. در این گام با توجه به اینکه در گام پیشین به کدام روش کدهای پروژه را ایجاد کرده‌اید باید یکی از روش‌های زیر را در پیش بگیرید.

### بازبینی و بازنویسی برای زبان فارسی

از آن‌جایی که طرح‌های ابزار stitch مناسب زبان‌های چپ به راست است باید کدهای پروژه را مناسب با زبان فارسی مورد بازبینی و بازنویسی قرار داد. برای این کار باید کدها را با استفاده از پرامپت زیر به یکی از دو ابزار [chatGPT](https://chatgpt.com) یا [Gemini 2.5 pro](https://gemini.google.com) داد تا آن‌‌ها کد را بررسی و تغییرات مورد نیاز در کد اعمال کنند.

> پرامپت: فایلی که بارگذاری کردم مربوط یک اپلیکیشن موبایل است. کد را بررسی کن و اگر بازنویسی لازم داره انجام بده. دقت کن که این اپلیکیشن برای زبان فارسی است.

این ابزارها به طور معمول باید تغییرات زیر را در کد اعمال کنند:

1. راست‌چین‌سازی (RTL): کل سند برای زبان فارسی راست‌چین شود.
2. ترجمه متن‌ها: تمام متون، از جمله عنوان صفحه، تیتر، گزینه‌ها، متن‌های پیش‌فرض و دکمه‌ها به فارسی ترجمه شوند.
3. فونت فارسی: افزودن فونت فارسی به کد. به طور معمول فونت «وزیرمتن» (Vazirmatn) افزوده می‌شود.
4. اصلاح آیکون: آیکون‌ها باید مناسب با زبان فارسی جایگزین شوند. نمونه: "فلش بازگشت" برای مطابقت با جهت راست‌چین
### بازبینی و بازنویسی

اگر کدهای خروجی براساس تصویر داده شده به stitch باشد می‌توان با دادن پرامپت زیر به یکی از ابزارهای [chatGPT](https://chatgpt.com) یا [Gemini 2.5 pro](https://gemini.google.com) آن‌ها را مورد بازبینی و بازنویسی قرار داد.

> پرامپت: فایلی که بارگذاری کردم مربوط یک اپلیکیشن موبایل است. کد را بررسی کن و اگر بازنویسی لازم داره انجام بده. دقت کن که هیچ تغییر در ویژگی‌ها داده نشه.

## یکپارچه‌سازی

در این گام می‌توان تمامی فایل‌های (HTML) پروژه را در یکی از ابزارهای زیر بارگذاری کرد و از آن‌ها خواست تا این صفحه‌ها را تبدیل به یک پروژه کامل کنند.

1. [aistudio google](https://aistudio.google.com)
2. [v0](https://v0.dev)
3. [bolt](https://bolt.new)
> پرامپت: فایل‌هایی که بارگذاری کردم مربوط به یک اپلیکیشن موبایل به زبان فارسی هست، کدها را بررسی و پروژه را یکپارچه‌سازی کن.

پس از یکپارچه‌سازی صفحه‌ها توسط مدل می‌توانید درخواست‌های بعدی خود را با استفاده از پرامپت به آن بدهید تا در پروژه اعمال کند.

## نکته‌ها

برای پیاده‌سازی محصول با استفاده از مدل‌ها و ابزارهای هوش بهتر است به موارد زیر توجه کنید.

### مشاور فنی

پیش از پیاده‌سازی بخش‌های پیچیده، ویژگی‌ها یا هر بخش از محصول می‌توانید از مدل‌های دیگر به عنوان مشاور فنی استفاده کنید. این کار باعث می‌شود دیده بهتری نسبت به فرآیند پیاده‌سازی و مشکل‌های احتمالی داشته باشید.

برای استفاده از مدل‌ها به عنوان مشاور فنی پیشنهاد می‌شود از وبگاه [lmarena](https://lmarena.ai) استفاده کنید. این وبگاه به شما اجازه می‌دهید با دو مدل به صورت همزمان گفتگو کرده و پاسخ آن‌ها را با یکدیگر مقایسه کنید.

### بهبود پرامپت

بهتر از پیش از پیاده‌سازی یک ویژگی جدید در محصول از مدل‌های دیگر بخواهید پرامپت شما را بررسی کنند و تا اگر نیاز به بهبود و تغییر دارد آن را اعمال کنند. این کار چند مزیت دارد:

- ساختار پرامپت را اصلاح می‌کنند تا برای مدل قابل درک باشد. این کار باعث شفاف‌تر و کامل‌تر شدن پرامپت شما می‌شود.
- مدل‌های هوش نکته‌های فنی خوبی ارائه می‌دهند.
- گاهی اوقات پیش می‌آید که منطق ویژگی شما مشکل دارد و مدل‌ها آن را برطرف می‌کنند.
- در برخی مواقع مدل‌ها دید گسترده‌تری دارند و نکته‌هایی را بیان می‌کنند که شما توجه‌ای به آن‌ها نداشته‌اید. (به طور نمونه جزئیات را بهتر توصیف می‌کنند یا برخی از نیازمندی‌های را به ویژگی درخواستی شما اضافه می‌کنند که کاربردی هستند.)
###  پیاده‌سازی بدون چهارچوب

 پیشنهاد می‌شود از مدل‌ها بخواهید تا بدون استفاده از هیچ چارچوبی (React, Vue و …) محصول را پیاده‌سازی کنند. این روش مزیت‌های زیر را دارد:

- کاهش حجم نهایی پروژه

- بدون نیاز به کامپایل

- قابلیت اجرای پروژه در محیط‌های مختلف

- کاهش وابستگی به کتابخانه‌های چارچوب‌ها

از آن‌جایی که ابزارهای ساخت محصول به صورت پیش‌فرض از چارچوب React برای پیاده‌سازی استفاده می‌کنند، در گام نخست خروجی آن‌ها React است و پس آن باید پرامپتی به آن‌ها بدهید و بخواهید تا از هیچ چارچوبی استفاده نکنند و به صورت خام (pure javascript) محصول را پیاده‌سازی کنند.

### معماری فنی

از ابتدای پیاده‌سازی محصول از مدل بخواهید آن را به صورت ماژولار و کد تمیز (clean code) پیاده‌سازی کند. این کار مزیت‌های زیر را دارد:

- دقت و سرعت پیاده‌سازی را افزایش می‌دهد.
- از ایجاد خطا و تداخل در ویژگی‌ها جلوگیری می‌کند.
- از حجیم شدن فایل‌ها جلوگیری می‌کند.

**نمونه پرامپت:**

پروژه را به صورت ماژولار با رعایت شرایط زیر بازنویسی کن:
ساختار پوشه‌بندی پیشنهادی:

```
project/
├── index.html
├── assets/
│   ├── css/
│   │   └── main.css
│   └── fonts/                (فونت‌های فارسی)
├── js/
│   ├── app.js               (Entry point - initialize)
│   ├── config.js            (تنظیمات عمومی)
│   ├── core/
│   ├── features/
│   └── utils/
└── libs/
```

نحوه import در HTML:

```
<!-- فقط app.js را import کن -->
<script type="module" src="js/app.js"></script>
<!-- CSS ها -->
<link rel="stylesheet" href="assets/css/main.css">
```

نکات مهم:

- از async/await برای عملیات ناهمزمان استفاده کن
- از try/catch برای مدیریت خطا استفاده کن
- هر فایل فقط یک مسئولیت داشته باشد (Single Responsibility)
- از نام‌گذاری فارسی در کامنت‌ها استفاده کن
- eventListener ها را به صورت متمرکز مدیریت کن
محدودیت‌ها:

- از هیچ کتابخانه جدیدی استفاده نکن
- ساختار HTML اصلی را تغییر نده (فقط script tags)
- از ES6 Modules استفاده کن

### پیاده‌سازی ویژگی

برای پیاده‌سازی یک ویژگی جدید نکته‌های زیر را مدنظر قرار دهید:

- در هر پرامپت یک ویژگی را توصیف کنید تا مدل بتواند آن را با اشتباه کمتری پیاده‌سازی کند.
  - در صورتیکه چند ویژگی را در یک پرامپت بنویسید احتمال خطا در پیاده‌سازی افزایش پیدا می‌کند و شما مجبور می‌شوید تا در پرامپت‌های بعدی بر روی رفع مشکلات تمرکز کنید.
- برای پیاده‌سازی یک ویژگی یا بهبود یکسان در چند بخش از پروژه بهتر است ابتدا به مدل بگویید در یک بخش کوچک پروژه تغییر را ایجاد کند و اگر تغییر مورد تایید بود، سپس به مدل بگویید آن را در تمام پروژه اعمال کند.
  - به طور نمونه برای افزودن جدا کننده سه رقم به فیلد اعداد، ابتدا باید از مدل خواست آن را در یک فیلد خاص پیاده‌سازی کند و اگر درست بود، سپس آن را در کل پروژه پیاده‌سازی کند.
- باید توجه داشت که برخی از ویژگی‌ها برای پیاده‌سازی نیازمند دانش معماری نرم‌افزار است و نمی‌توان با چند پرامپت ساده آن‌ها را پیاده‌سازی کرد.

### استفاده از کتابخانه‌ها و افزونه‌ها

اگر می‌خواهید در محصول خود از کتابخانه‌ها یا افزونه‌ها استفاده کنید بهتر است متن نحوه استفاده از آن را همراه با پرامپت خود ارسال کنید. این کار باعث می‌شود دقت مدل در پیاده‌سازی آن کتابخانه یا افزونه در محصول افزایش پیدا کند.

- نحوه استفاده از یک کتابخانه یا افزونه را می‌توانید در سندهای آن یا فایل README موجود در گیت‌هاب بیابید.
- اگر در سندها مثالی برای پیاده‌سازی وجود دارد آن را هم به مدل بدهید.
- پیش از اینکه از مدل بخواهید از کتابخانه و افزونه استفاده کنید، حتما مخزن و سندهای آن را بررسی کنید تا مطمئن شوید که مدل از نسخه یکسان برای پیاده‌سازی و بررسی سندها استفاده می‌کند. زیرا مدل‌های هوش به احتمال زیاد با نسخه‌های قدیمی کتابخانه یا افزونه آموزش دیده‌اند.

### سند نویسی

از همان ابتدای پیاده‌سازی از مدل بخواهید سندهای محصول را بنویسد.

- سندهای مورد نیاز یک پروژه می‌تواند شامل موارد زیر باشد:
  - راهنمای کاربر
  - معماری محصول (فنی)
  - سند محصول (سندی که ویژگی‌ها و منطق محصول را توصیف می‌کند.)
  - پرونده README
  - تاریخچه تغییرات (changelog)
- دقت کنید که پس از هر ویژگی باید سندها بروزرسانی شوند.